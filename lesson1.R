x <- c(10.4, 5.6, 3.1, 6.4, 21.7)  # skapar en vektor
assign("x", c(10.4, 5.6, 3.1, 6.4, 21.7)) # samma sak med assign funktionen

1/x  # delar 1 med vektorn x och skriver ut värdena, påverkar inte vad som finns i x

y <- c(x, 0, x)   #y gör en vektor som slår ihop vektorn x med 0 och x igen

n <- 10  # n tilldelas 10

temp <- x > 13   # temp skapar logicals (booleans) för vektorn X TRUE om över 13 annars FALSE
temp

y <- x[!is.na(x)]  # y = x utan NotAvailable nummer

c("x","y")[rep(c(1,2,2,1), times=4)] #produces a character vector of length 16 consisting of "x", "y", "y", "x" repeated four times.

y <- x[-(1:5)] #gives y all but the first five elements of x. minustecknet negerar uttrycket 

fruit <- c(5, 10, 1, 20)
names(fruit) <- c("orange", "banana", "apple", "peach")  #alfanumeriska namn
...
scan() #tar input, obegränsat tills dubbel enter



x <- c(10, 5, 100, 2, 65, 13)   # skapa vektor
x
class(x)

y <- list("Janne", 10, 1.1, "Titti", TRUE) #skapa lista med eller utan namn på elementen se rad nedan
y <- list(namn = "Janne", ålder = 45)   
y
class(y)

z <- data.frame(                                 #skapa dataframe
  name = c("Janne", "Titti"),
  age = c(42, 35)
)
z
